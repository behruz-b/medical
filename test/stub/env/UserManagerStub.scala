...
import protocols.UserProtocol._
...
class UserManagerStub @Inject()(val configuration: Configuration,
  ...
  private def changePassword(login: String, newPass: String): Future[Either[String, String]] = {
    repo.changePassword(login, md5(newPass)).unsafeToFuture().map { result =>
      if (result == 1) {
  ...
  private def checkUserByLoginAndPassword(login: String, password: String): Future[Either[String, String]] = {
    repo.getUserByLogin(login).compile.last.unsafeToFuture().map { result =>
  ...
  private def checkUserByLoginAndCreate(user: User): Future[Either[String, String]] = {
    repo.createUser(user.copy(password = md5(user.password))).unsafeToFuture().map { _ =>
